...
#              fig, ax = plt.subplots(figsize=(1024*px, 1024*px), facecolor='black',
                       subplot_kw=dict(polar=True))
#              fig = plt.figure(figsize=(800*px, 800*px))
              title=input_fname_name+'_inverse_circle_'+methods[0]+'_'+f'{bands[0][0]:.1f}'+'-'+f'{bands[0][len(bands[0])-1]:.1f}'+'hz_'+'vmin'+str(vmin)+'_'+'parc-'+inverse_parc+'\n'+f'{ji_fps:.2f}'
...
#                          exclude=(), preload=False, verbose=True)
  ...
  if (FLAGS.write_video):
    ...
    if FLAGS.video_output_file==None:
      ...
      if show_inverse_circle_cons:
        video_output_file=output_path+input_name+'_'+shows[shows_inverse_circle]+'_'+methods[0]+'_'+f'{bands[0][0]:.1f}'+'-'+f'{bands[0][len(bands[0])-1]:.1f}'+'hz_'+'vmin'+str(vmin)+'_'+'parc-'+inverse_parc+"_"+dt_string+".mp4"
